...
//    private var listener: OnListFragmentInteractionListener? = null
    ...
    override fun onCreateView(
        ...
        if (recyclerView is RecyclerView) {
            if (auth.uid != null) {
                ordersRef.whereEqualTo(Constants.CUSTOMER_ID, auth.uid).get().addOnSuccessListener {

                    val order = Order.fromSnapshot(it.documents[0])
                    view.txt_order_details_customer_address.text = "Delivery address: ${order.customerAddress}"
                    view.txt_order_details_customer_name.text = "Customer name: ${order.customerName}"
                    view.txt_order_details_customer_phone_number.text = "Phone number: ${order.customerPhone}"
